...
public class TcpSuperSenseProcessor implements HealthCheckProcessor, Runnable {
    ...
    public void process(HealthCheckTask task) {
        ...
        for (Instance ip : ips) {
            ...
            if (!ip.markChecking()) {
                SRV_LOG.warn("tcp check started before last one finished, com.yuanxiaoyuan.springcloudalibabafeign.service: "
...
//        selector.wakeup();
    ...
    public class PostProcessor implements Runnable {
        ...
        public void run() {
            ...
            try {
                if (!beat.isHealthy()) {
                    //invalid beat means this server is no longer responsible for the current com.yuanxiaoyuan.springcloudalibabafeign.service
                    key.cancel();
                    key.channel().close();

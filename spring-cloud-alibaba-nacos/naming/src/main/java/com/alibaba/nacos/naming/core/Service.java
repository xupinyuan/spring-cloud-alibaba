 ...
 * <p>
 * We introduce a 'com.yuanxiaoyuan.springcloudalibabafeign.service --> cluster --> instance' model, in which com.yuanxiaoyuan.springcloudalibabafeign.service stores a list of clusters,
...
public class Service extends com.alibaba.nacos.api.naming.pojo.Service implements Record, RecordListener<Instances> {
    ...
    public void update(Service vDom) {
        ...
        if (!StringUtils.equals(token, vDom.getToken())) {
            Loggers.SRV_LOG.info("[SERVICE-UPDATE] com.yuanxiaoyuan.springcloudalibabafeign.service: {}, token: {} -> {}", getName(), token, vDom.getToken());
            token = vDom.getToken();
        }

        if (!ListUtils.isEqualList(owners, vDom.getOwners())) {
            Loggers.SRV_LOG.info("[SERVICE-UPDATE] com.yuanxiaoyuan.springcloudalibabafeign.service: {}, owners: {} -> {}", getName(), owners, vDom.getOwners());
            owners = vDom.getOwners();
        }

        if (getProtectThreshold() != vDom.getProtectThreshold()) {
            Loggers.SRV_LOG.info("[SERVICE-UPDATE] com.yuanxiaoyuan.springcloudalibabafeign.service: {}, protectThreshold: {} -> {}", getName(), getProtectThreshold(), vDom.getProtectThreshold());
            setProtectThreshold(vDom.getProtectThreshold());
        }

        if (resetWeight != vDom.getResetWeight().booleanValue()) {
            Loggers.SRV_LOG.info("[SERVICE-UPDATE] com.yuanxiaoyuan.springcloudalibabafeign.service: {}, resetWeight: {} -> {}", getName(), resetWeight, vDom.getResetWeight());
            resetWeight = vDom.getResetWeight();
        }

        if (enabled != vDom.getEnabled().booleanValue()) {
            Loggers.SRV_LOG.info("[SERVICE-UPDATE] com.yuanxiaoyuan.springcloudalibabafeign.service: {}, enabled: {} -> {}", getName(), enabled, vDom.getEnabled());
    ...
    public synchronized void recalculateChecksum() {
        ...
        if (Loggers.SRV_LOG.isDebugEnabled()) {
            Loggers.SRV_LOG.debug("com.yuanxiaoyuan.springcloudalibabafeign.service to json: " + getServiceString());
        }

...
/**
 * Health status related operation com.yuanxiaoyuan.springcloudalibabafeign.controller
 *
 * @author nkorange
 * @author nanamikon
 * @since 0.8.0
            ...
            for (Instance instance : service.allIPs(Lists.newArrayList(clusterName))) {
                if (instance.getIp().equals(ip) && instance.getPort() == port) {
                    ...
                    Loggers.EVT_LOG.info((valid ? "[IP-ENABLED]" : "[IP-DISABLED]") + " ips: "
                        ...
                        + ", com.yuanxiaoyuan.springcloudalibabafeign.service: " + serviceName + ", msg: update thought HealthController api");
                    pushService.serviceChanged(namespaceId, service.getName());
                    break;
                }
            }
        } else {
            throw new IllegalArgumentException("health check is still working, com.yuanxiaoyuan.springcloudalibabafeign.service: " + serviceName);
        }

    ...
    public void doAuth(Map<String, String[]> params, HttpServletRequest req) throws Exception {
        ...
        if (service == null) {
            if (!req.getRequestURI().equals(UtilsAndCommons.NACOS_NAMING_CONTEXT + UtilsAndCommons.API_SET_ALL_WEIGHTS)) {
                throw new IllegalStateException("auth failed, com.yuanxiaoyuan.springcloudalibabafeign.service does not exist: " + serviceName);
            }
        }
        ...
        if (StringUtils.isEmpty(auth) && StringUtils.isEmpty(token)) {
            throw new IllegalArgumentException("provide 'authInfo' or 'token' to access this com.yuanxiaoyuan.springcloudalibabafeign.service");
        }
